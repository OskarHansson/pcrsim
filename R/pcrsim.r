...
# 16.01.2022: Migrated to gWidgets2.
...
pcrsim <- function(debug = FALSE) {
  ...
  nb <- gnotebook(
    closebuttons = FALSE, dontCloseThese = NULL,
  ...
  # Define groups.
  start_gf <- ggroup(
    horizontal = FALSE, spacing = 10, use.scrollwindow = FALSE,
    container = nb, label = .start_tab_name, expand = TRUE
  )

  project_gf <- ggroup(
    horizontal = FALSE, spacing = 10, use.scrollwindow = FALSE,
    container = nb, label = .project_tab_name
  )
  ...
  start_f1 <- gframe(
    text = "PCR sim", markup = FALSE, pos = 0, horizontal = TRUE,
  ...
  about_txt <- paste("PCRsim is a package for simulation of the forensic ",
    ...
    "https://github.com/OskarHansson/pcrsim",
    sep = ""
  )
  ...
  start_f2 <- gframe(
    text = "License", markup = FALSE, pos = 0, horizontal = TRUE,
    container = start_gf, expand = TRUE
  )
  ...
  gtext(
    text = license_txt, width = NULL, height = NULL, font.attr = NULL,
  ...
  project_fb <- gfilebrowse(
    text = getwd(), type = "selectdir", quote = FALSE,
  ...
  # Horizontal main group.
  project_f3 <- gframe(
    text = "Description", horizontal = TRUE, spacing = 10,
    container = project_f1, expand = TRUE
  )

  # Button group.
  project_g3 <- ggroup(
    horizontal = FALSE, spacing = 10, container = project_f3,
    expand = FALSE
  )
  ...
  file_list <- strvalidator::listObjects(
    env = .pcrsim_env,
  ...
  file_f1 <- gframe(
    text = "Project workspace", markup = FALSE, pos = 0,
    horizontal = TRUE, container = file_gf, expand = TRUE
  )

  file_f1g1 <- ggroup(horizontal = FALSE, container = file_f1, expand = FALSE)

  file_f1g1_load_btn <- gbutton(text = "Load", container = file_f1g1)
  tooltip(file_f1g1_load_btn) <- "Load project"

  file_f1g1_ws_save_btn <- gbutton(text = "Save", container = file_f1g1)
  tooltip(file_f1g1_ws_save_btn) <- "Save project"

  file_f1g1_ws_saveas_btn <- gbutton(text = "Save As", container = file_f1g1)
  tooltip(file_f1g1_ws_saveas_btn) <- "Save project in specified location"

  file_f1g1_import_btn <- gbutton(text = "Import", container = file_f1g1)
  tooltip(file_f1g1_import_btn) <- "Import from file"

  file_f1g1_refresh_btn <- gbutton(text = "Refresh", container = file_f1g1)
  tooltip(file_f1g1_refresh_btn) <- "Refresh project workspace"

  file_f1g1_view_btn <- gbutton(text = "View", container = file_f1g1)
  tooltip(file_f1g1_view_btn) <- "View dataset or profile"

  file_f1g1_rm_btn <- gbutton(text = "Remove", container = file_f1g1)
  tooltip(file_f1g1_rm_btn) <- "Remove selected object"

  file_f1g1_rename_btn <- gbutton(text = "Rename", container = file_f1g1)
  tooltip(file_f1g1_rename_btn) <- "Rename selected object"

  file_f1g1_save_chk <- gcheckbox(
  ...
  file_f1g1_ext_debug_chk <- gcheckbox(
    text = "Extended debug",
    checked = FALSE,
    container = file_f1g1
  )
  ...
  file_f1g1_save_temp_btn <- gbutton(text = "Save", container = file_f1g1)
  tooltip(file_f1g1_save_temp_btn) <- "Save simulation settings"

  file_f1g1_load_temp_btn <- gbutton(text = "Load", container = file_f1g1)
  ...
  file_f1g1_tbl <- gtable(
    items = file_list, multiple = TRUE,
  ...
  addHandlerChanged(file_f1g1_rename_btn, handler = function(h, ...) {
    ...
    if (length(object) == 1) {
      ...
      gmessage(
        message = "You can only rename one object at a time!",
        title = "Error",
        icon = "error",
        parent = w
      )
  ...
  addHandlerChanged(file_f1g1_load_btn, handler = function(h, ...) {
    ws_path <- gfile(
      text = "Select a saved project", type = "open",
  ...
  addHandlerChanged(file_f1g1_view_btn, handler = function(h, ...) {
    ...
    if (!is.null(val_obj) && !is.na(val_obj) && length(val_obj) > 0) {
      if ("data.frame" %in% val_class) {
        ...
        # Open GUI.
        strvalidator::editData_gui(
          env = .pcrsim_env, data = val_data,
  ...
  addHandlerChanged(file_f1g1_ws_save_btn, handler = function(h, ...) {
    ...
    if (ok) {
      if (!is.na(ws_name) && !ws_name == "") {
        ...
        gmessage(
          message = "A file name must be given",
      ...
      gmessage(
        message = "No project name or path!\nUse 'Save As' instead.",
  ...
  addHandlerChanged(file_f1g1_save_temp_btn, handler = function(h, ...) {
    ...
    # Ask for name.
    valName <- ginput(
      message = "Name for simulation settings template", text = "",
  ...
  file_f2 <- gframe(
    text = "Load dataframe from R workspace",
    markup = FALSE,
    pos = 0,
    horizontal = TRUE,
    container = file_gf,
  ...
  file_f2g1_refresh_btn <- gbutton(text = "Refresh", container = file_f2g1)
  tooltip(file_f2g1_refresh_btn) <- "Refresh dropdown"

  file_f2g1_load_btn <- gbutton(text = "Load", container = file_f2g1)
  tooltip(file_f2g1_load_btn) <- "Load dataset"

  file_f2g1_drp <- gcombobox(
    items = c(
      "<Select dataframe>",
      strvalidator::listObjects(
        env = .pcrsim_env,
  ...
  profile_f1g1[2, 1] <- glabel(
    text = "The following kit will be used:",
    container = profile_f1g1, anchor = c(-1, 0)
  )

  profile_f1g1[2, 2] <- profile_kit_drp <- gcombobox(
    items = getParameter(),
    selected = 1,
    editable = FALSE,
    ellipsize = "none",
  ...
  profile_f1g1[2, 3] <- glabel(
    text = "with settins from method:",
    container = profile_f1g1, anchor = c(-1, 0)
  )

  profile_f1g1[2, 4] <- profile_method_drp <- gcombobox(
    items = getParameter(what = "Methods"),
    selected = 1,
    editable = FALSE,
    ellipsize = "none",
    container = profile_f1g1
  ...
  profile_f2 <- gframe(
    text = "Create profile", markup = FALSE, pos = 0,
  ...
  profile_f2g1[2, 1:2] <- profile_opt <- gradio(
  ...
  profile_f2g1[3, 1] <- glabel(
    text = "Load from R workspace:",
    container = profile_f2g1, anchor = c(-1, 0)
  )
  
  if(is.null(wsObj)){
    # If using gWidgets2tcltk: Error on next line if wsObj=NULL 
    wsObj <- c("No objects in workspace")
  }
  
  profile_f2g1[3, 2] <- profile_ws_drp <- gcombobox(
    items = wsObj,
    selected = 1,
    editable = FALSE,
    ellipsize = "none",
  ...
  profile_f2g1[3, 3] <- profile_ws_btn <- gbutton(text = "Select", container = profile_f2g1)
  enabled(profile_ws_btn) <- FALSE

  profile_f2g1[4, 1] <- glabel(
    text = "Import from file:",
    container = profile_f2g1, anchor = c(-1, 0)
  )
  
  profile_f2g1[4, 2:3] <- profile_file_browser <- gfilebrowse(
    text = profileText,
    quote = FALSE,
    type = "open",
    container = profile_f2g1
  ...
  profile_f2g1[5, 1] <- glabel(
    text = "Population database:",
    container = profile_f2g1, anchor = c(-1, 0)
  )

  profile_f2g1[5, 2] <- profile_pop_drp <- gcombobox(
    items = profileDb,
    selected = 1,
    editable = FALSE,
    ellipsize = "none",
  ...
  profile_f2g1[5, 3] <- profile_generate_btn <- gbutton(
    text = "Generate",
    container = profile_f2g1
  )
  ...
  addHandlerChanged(profile_ws_btn, handler = function(h, ...) {
    ...
    if (!is.null(val_obj) && !is.na(val_obj)) {
      ...
      if (all(c("Marker", "Allele.1", "Allele.2") %in% names(dnaProfile))) {
        ...
        dnaProfile <- strvalidator::trim(
          data = dnaProfile, samples = NULL,
  ...
  addHandlerChanged(profile_file_browser, handler = function(h, ...) {
    ...
    if (enabled(profile_file_browser)) {
      if (file.exists(val)) {
        dnaProfile <- strvalidator::import(
          import.file = val, time.stamp = FALSE,
  ...
  addHandlerChanged(profile_generate_btn, handler = function(h, ...) {
    ...
    if (!is.na(val_db) && !is.null(val_kit)) {
      ...
      # Simulate sample.
      generatedData <- simProfile(
        data = NULL, sim = 1, kit = val_kit,
  ...
  profile_f3 <- gframe(
    text = "View and edit profile", markup = FALSE, pos = 0,
  ...
  profile_profile_item <- data.frame(
    Marker = markerList,
    Allele.1 = "NA",
    Allele.2 = "NA",
    stringsAsFactors = FALSE
  ...
  profile_f3g1[1, 1] <- profile_tbl <- gdf(
    items = profile_profile_item,
  ...
  sample_f1g1[2, 1] <- glabel(
    text = "Sample name:",
    container = sample_f1g1, anchor = c(-1, 0)
  )

  sample_f1g1[2, 2] <- sample_name_edt <- gedit(
    text = "",
    width = sample_f1_txt_width,
    container = sample_f1g1
  )

  sample_f1g1[3, 1:2] <- sample_mix_chk <- gcheckbox(
    text = "", checked = FALSE,
    container = sample_f1g1, anchor = c(-1, 0)
  )
  ...
  sample_by_opt <- gradio(
    items = sample_by_items, selected = 1,
  ...
  sample_f2g1[2, 1] <- glabel(
    text = "Number of cells:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[2, 2] <- sample_ncells_edt <- gedit(
    text = "",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[2, 3] <- glabel(
    text = "Standard deviation:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[2, 4] <- sample_ncells_sd_edt <- gedit(
    text = "0",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[3, 1] <- glabel(
    text = "DNA concentration (ng/\u00B5l):",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[3, 2] <- sample_conc_edt <- gedit(
    text = "",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[3, 3] <- glabel(
    text = "Standard deviation:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[3, 4] <- sample_conc_sd_edt <- gedit(
    text = "0",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )
  ...
  sample_f2g1[4, 1] <- glabel(
    text = "DNA per diploid cell (pg):",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[4, 2] <- sample_celldna_edt <- gedit(
    text = "6",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )
  ...
  sample_f2g1[5, 1] <- glabel(
    text = "Sample volume (\u00B5l):",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[5, 2] <- sample_vol_edt <- gedit(
    text = "",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[5, 3] <- glabel(
    text = "Standard deviation:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[5, 4] <- sample_vol_sd_edt <- gedit(
    text = "0",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[6, 1] <- glabel(
    text = "Slope:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[6, 2] <- sample_slope_edt <- gedit(
    text = "-0.008",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[6, 3] <- glabel(
    text = "Intercept:",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[6, 4] <- sample_intercept_edt <- gedit(
    text = "2.5",
    width = sample_f2_txt_width,
    container = sample_f2g1
  )

  sample_f2g1[7, 1] <- glabel(
    text = "Concentration @ 100bp (ng/\u00B5l):",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[7, 2] <- sample_ex1_edt <- gedit(
    text = "", width = sample_f2_txt_width,
    container = sample_f2g1
  )
  enabled(sample_ex1_edt) <- FALSE


  sample_f2g1[8, 1] <- glabel(
    text = "Concentration @ 400bp (ng/\u00B5l):",
    container = sample_f2g1, anchor = c(-1, 0)
  )

  sample_f2g1[8, 2] <- sample_ex2_edt <- gedit(
    text = "", width = sample_f2_txt_width,
    container = sample_f2g1
  )
  ...
  sample_f2g1[9, 3:4] <- sample_type_cmb <- gcombobox(
  ...
  deg_f1 <- gframe(
    text = "Degradation", markup = FALSE, pos = 0,
  ...
  deg_f1g1[1, 1] <- glabel(
    text = "Current P(deg)=", container = deg_f1g1,
    anchor = c(1, 0)
  )
  deg_f1g1[1, 2] <- deg_pam_lbl <- glabel(text = "NA", container = deg_f1g1)

  deg_f1g1[2, 1] <- glabel(
    text = "Quantification amplicon size (bp):",
    container = deg_f1g1, anchor = c(1, 0)
  )
  deg_f1g1[2, 2] <- deg_target_edt <- gedit(
    text = "80", container = deg_f1g1,
    width = deg_f1_txt_w
  )
  ...
  deg_f1g1[3:3, 1] <- deg_opt <- gradio(
    items = deg_tmp_items, selected = 1,
    horizontal = FALSE, container = deg_f1g1
  )

  deg_f1g1[4, 1] <- glabel(
    text = "Degradation parameter (probability per bp):",
    container = deg_f1g1, anchor = c(-1, 0)
  )

  deg_f1g1[4, 2] <- deg_spn <- gspinbutton(
  ...
  deg_f1g1[5, 1] <- glabel(
    text = "Concentration in ng/\u00B5l (separated by comma):",
    container = deg_f1g1, anchor = c(-1, 0)
  )

  deg_f1g1[5, 2] <- deg_conc_edt <- gedit(
    text = "", width = deg_f1_txt_w,
    container = deg_f1g1
  )

  deg_f1g1[6, 1] <- glabel(
    text = "Size in base pair (separated by comma):",
    container = deg_f1g1, anchor = c(-1, 0)
  )

  deg_f1g1[6, 2] <- deg_size_edt <- gedit(
    text = "", width = deg_f1_txt_w,
    container = deg_f1g1
  )
  ...
  ex_grid_1[2, 3] <- glabel(
    text = "Standard deviation:",
    container = ex_grid_1, anchor = c(-1, 0)
  )

  ex_grid_1[2, 4] <- ex_eff_sd_edt <- gedit(
    text = "0",
  ...
  ex_grid_2[2, 1] <- glabel(
    text = "Extraction volume (\u00B5l):",
    container = ex_grid_2, anchor = c(-1, 0)
  )

  ex_grid_2[2, 2] <- ex_vol_edt <- gedit(
    text = "100",
    width = ex_frame_2_txt_width,
    container = ex_grid_2
  )

  ex_grid_2[2, 3] <- glabel(
    text = "Standard deviation:",
    container = ex_grid_2, anchor = c(-1, 0)
  )

  ex_grid_2[2, 4] <- ex_vol_sd_edt <- gedit(
    text = "0",
    width = ex_frame_2_txt_width,
    container = ex_grid_2
  )
  ...
  dil_f2g1[1:2, 1] <- dil_target_opt <- gradio(
    items = dil_tmp_items, selected = 1,
  ...
  dil_f2g1[2, 2] <- dil_target_edt <- gedit(
    text = "",
  ...
  dil_f2g1[3, 1] <- glabel(
    text = "Tolerance for target:",
    container = dil_f2g1, anchor = c(-1, 0)
  )

  dil_f2g1[3, 2] <- dil_tolerance_edt <- gedit(
    text = "0",
    width = dil_f2_txt_width,
    container = dil_f2g1
  )
  ...
  dil_f3g1[1, 1] <- glabel(
    text = "Minimum pipetting volume:",
    container = dil_f3g1
  )


  dil_f3g1[1, 2] <- dil_accuracy_edt <- gedit(
    text = "1",
  ...
  dil_f4g1[1:2, 1] <- dil_volume_opt <- gradio(
    items = dil_tmp_items, selected = 1,
  ...
  dil_f4g1[2, 2] <- dil_volume_edt <- gedit(
    text = "",
  ...
  amp_f1g1[2, 1] <- glabel(
    text = "Aliquot for PCR (\u00B5l):",
    container = amp_f1g1, anchor = c(-1, 0)
  )

  amp_f1g1[2, 2] <- amp_aliquot_edt <- gedit(
    text = "10",
  ...
  amp_f1g1[2, 4] <- amp_aliquot_sd_edt <- gedit(
    text = "0",
    width = amp_f1_txt_width,
    container = amp_f1g1
  )
  ...
  amp_f2g1[2, 1] <- glabel(
    text = "Optimal amount of DNA:",
    container = amp_f2g1, anchor = c(-1, 0)
  )

  amp_f2g1[2, 2] <- amp_amount_edt <- gedit(
    text = "0.5",
  ...
  amp_f2g1[3, 1] <- glabel(
    text = "Total amplification volume:",
    container = amp_f2g1, anchor = c(-1, 0)
  )

  amp_f2g1[3, 2] <- amp_total_vol_edt <- gedit(
    text = "25",
    width = amp_f2_txt_width,
    container = amp_f2g1
  )
  ...
  amp_f2g1[3, 4] <- amp_total_vol_sd_edt <- gedit(
    text = "0",
    width = amp_f2_txt_width,
    container = amp_f2g1
  )
  ...
  amp_f2g1[4, 2] <- cyc_sb <- gspinbutton(
    from = 1, to = 50, by = 1,
  ...
  amp_f3_eff_opt <- gradio(
    items = amp_f3_items, selected = 1,
  ...
  amp_f3g1[2, 1] <- glabel(
    text = "Amplification probability:",
    container = amp_f3g1, anchor = c(-1, 0)
  )

  amp_f3_eff_spn <- gspinbutton(
    from = 0, to = 1, by = 0.001,
  ...
  amp_f3g1[2, 4] <- amp_f3_eff_sd_edt <- gedit(
    text = "0",
  ...
  amp_f3g1[3, 1] <- glabel(
    text = "Stutter probability:",
    container = amp_f3g1, anchor = c(-1, 0)
  )

  amp_eff_stutt_spn <- gspinbutton(
    from = 0, to = 1, by = 0.001,
  ...
  amp_f3g1[3, 3] <- glabel(
    text = "Standard deviation:", container = amp_f3g1,
    anchor = c(-1, 0)
  )

  amp_eff_stutt_sd_edt <- gedit(
    text = "0", width = amp_f3_txt_width,
    container = amp_f3g1
  )
  amp_f3g1[3, 4] <- amp_eff_stutt_sd_edt


  amp_stutter_chk <- gcheckbox(
    text = "Simulate stutters",
  ...
  ce_f2 <- gframe(
    text = "Capillary electrophoresis",
    markup = FALSE,
    pos = 0,
    horizontal = TRUE,
    container = ce_gf,
  ...
  ce_f2g1[2, 1] <- glabel(
    text = "Volume PCR product analysed (\u00B5l):",
    container = ce_f2g1, anchor = c(-1, 0)
  )

  ce_f2g1[2, 2] <- ce_aliquot_edt <- gedit(
    text = "1",
  ...
  ce_f2g1[2, 4] <- ce_aliquot_sd_edt <- gedit(
    text = "0",
    width = ce_f2_txt_width,
    initial.msg = "",
    container = ce_f2g1
  ...
  ce_settings_opt <- gradio(
    items = c(
      "Load threshold and scaling from parameter file",
  ...
  ce_f3g1[2, 1] <- glabel(
    text = "Threshold intercept:",
    container = deg_f1g1, anchor = c(-1, 0)
  )

  ce_f3g1[2, 2] <- ce_t_intercept_edt <- gedit(
    text = "13.9880",
  ...
  ce_f3g1[3, 1] <- glabel(
    text = "Threshold slope:",
    container = ce_f3g1, anchor = c(-1, 0)
  )

  ce_f3g1[3, 2] <- ce_t_slope_edt <- gedit(
    text = "0.9047",
    width = ce_f3_txt_width,
    container = ce_f3g1
  )
  ...
  ce_f3g1[4, 2] <- ce_t_residual_edt <- gedit(
    text = "0.5951",
    width = ce_f3_txt_width,
    container = ce_f3g1
  )
  ...
  ce_f1 <- gframe(
    text = "Peak height scaling", markup = FALSE, pos = 0,
  ...
  ce_f1g1[2, 1] <- glabel(
    text = "Scaling intercept:",
    container = deg_f1g1, anchor = c(-1, 0)
  )

  ce_f1g1[2, 2] <- ce_intercept_edt <- gedit(
    text = "-10.4812",
  ...
  ce_f1g1[3, 2] <- ce_slope_edt <- gedit(
    text = "0.8590",
    width = ce_f1_txt_width,
    container = ce_f1g1
  )
  ...
  ce_f1g1[4, 2] <- ce_residual_edt <- gedit(
    text = "0.5798",
    width = ce_f1_txt_width,
    container = ce_f1g1
  )
  ...
  sim_f1 <- gframe(
    text = "Options", markup = FALSE, pos = 0, horizontal = FALSE,
  ...
  sim_f1g1[2, 1] <- glabel(
    text = "Number of simulations or contributors:",
    container = sim_f1g1, anchor = c(-1, 0)
  )

  sim_f1g1[2, 2] <- sim_sim_edt <- gedit(
    text = "1", width = 10,
    container = sim_f1g1
  )
  ...
  sim_f1g1[3, 2] <- sim_name_edt <- gedit(
    text = "sim", width = 25,
    container = sim_f1g1
  )
  ...
  sim_f1g1[4, 2] <- sim_epg_title_edt <- gedit(
    text = "Simulated data",
    width = 25, container = sim_f1g1
  )


  sim_f1g1[5, 1] <- sim_rnd_seed_chk <- gcheckbox(
    text = "Set random seed:",
    checked = FALSE,
    container = sim_f1g1
  )
  sim_f1g1[5, 2] <- sim_rnd_seed_edt <- gedit(
    text = "", width = 6,
    container = sim_f1g1
  )

  sim_update_chk <- gcheckbox(
    text = "Auto update EPG",
    checked = FALSE, container = sim_f1
  )

  sim_save_chk <- gcheckbox(
    text = "Save in workspace, create name from:",
    checked = FALSE, container = sim_f1
  )

  sim_save_s_chk <- gcheckbox(
    text = "Sample Name",
    checked = FALSE, container = sim_f1
  )
  sim_save_t_chk <- gcheckbox(
    text = "Time stamp",
    checked = FALSE, container = sim_f1
  )
  sim_save_n_chk <- gcheckbox(
    text = "Result name",
    checked = FALSE, container = sim_f1
  )
  ...
  sim_f2 <- gframe(
    text = "Simulate", markup = FALSE, pos = 0, horizontal = FALSE,
    container = sim_gf, expand = FALSE
  )

  sim_profile_chk <- gcheckbox(
    text = "Profile",
    checked = FALSE, container = sim_f2
  )

  sim_sample_chk <- gcheckbox(
    text = "Sample",
    checked = FALSE, container = sim_f2
  )

  sim_deg_chk <- gcheckbox(
    text = "Degradation",
    checked = FALSE, container = sim_f2
  )

  sim_extr_chk <- gcheckbox(
    text = "Extraction",
    checked = FALSE, container = sim_f2
  )

  sim_norm_chk <- gcheckbox(
    text = "Normalization",
    checked = FALSE, container = sim_f2
  )

  sim_pcr_chk <- gcheckbox(
    text = "PCR Amplification",
    checked = FALSE, container = sim_f2
  )

  sim_ce_chk <- gcheckbox(
    text = "Capillary Electrophoresis",
    checked = FALSE, container = sim_f2
  )
  ...
  sim_sim_btn <- gbutton(text = "Simulate", container = sim_gf)

  sim_view_btn <- gbutton(text = "View result of the DNA process", container = sim_gf)

  sim_view_ph_btn <- gbutton(text = "View result after capillary electrophoresis", container = sim_gf)

  sim_epg_btn <- gbutton(text = "Generate EPG", container = sim_gf)
  ...
  addHandlerChanged(sim_sim_btn, handler = function(h, ...) {
    ...
    if (val_sim_profile) {
      ...
      # Simulate profile.
      simData <- simProfile(
        data = simData, kit = NULL, sim = val_simulations,
    ...
    if (val_sim_degradation) {
      ...
      # Simulate degradation.
      simData <- simDegradation(
        data = simData, kit = val_kit, deg = val_deg_deg,
    ...
    if (val_sim_dilution) {
      ...
      # Simulate dilution.
      simData <- simNormalize(
        data = simData, volume = val_dil_vol,
    ...
    if (val_sim_pcr) {
      ...
      simData <- simPCR(
        data = simData, kit = pcr_kit, method = pcr_method, pcr.cyc = val_amp_cyc,
    ...
    if (any(grepl("PCR.Stutter.", names(simData), fixed = TRUE))) {
      simData <- compactStutter(
        data = simData, stutterin = "PCR.Stutter.",
    ...
    if (any(grepl("PCR.Amplicon", names(simData)))) {
      simCompact <- compact(
        data = simData, per.sample = !valMixture,
    ...
    if (val_sim_ce) {
      ...
      # Simulate capillary electrophoresis.
      simPh <- simCE(
        data = simCompact, vol = val_ce_aliq, sd.vol = val_ce_aliq_sd,
    ...
    if (svalue(sim_update_chk) && !is.null(.simDataPh)) {
      # Generate EPG.
      .simEPG <<- strvalidator::generateEPG(
        data = .simDataPh,
        kit = val_kit,
        title = val_title,
        wrap = val_epg_wrap,
  ...
  addHandlerChanged(sim_view_btn, handler = function(h, ...) {
    if (!is.null(.simDataRes)) {
      ...
      gmessage(
        message = "Currently no data!", title = "Error",
        icon = "error", parent = w
      )
    }
  })
  ...
  .refreshSampleTab <- function() {
    ...
    labTmp <- paste(
      "Simulate a mixture with", valSim,
  ...
  .refreshLoaded <- function() {
    ...
    # Get data frames.
    dfs <- strvalidator::listObjects(env = .pcrsim_env, obj.class = c(
      "data.frame", "ggplot",
      "environment"
    ))
